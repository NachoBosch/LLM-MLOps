...
# Free Tier: 5GB almacenamiento, 20K GET, 2K PUT requests/mes (primer año)
resource "aws_s3_bucket" "memory" {
  bucket = "${local.name_prefix}-memory-${data.aws_caller_identity.current.account_id}"
  tags   = local.common_tags
}

# Lifecycle rule para reducir costos - eliminar objetos antiguos después de 90 días
...
# Lifecycle rule para versiones antiguas del frontend
...
# Lambda function
# Free Tier: 1M requests/mes y 400,000 GB-segundos/mes (permanente)
# Configurado con 128MB de memoria para maximizar tiempo de cómputo gratis
resource "aws_lambda_function" "api" {
  ...
  memory_size      = 128  # Mínimo para Free Tier (128MB = ~3,125,000 invocaciones de 1 segundo gratis/mes)
...
# Free Tier: 1M requests/mes gratis (permanente)
...
# Free Tier: 50GB transferencia saliente y 2M requests HTTP/HTTPS/mes (primer año)
resource "aws_cloudfront_distribution" "main" {
  ...
  default_cache_behavior {
    ...
    min_ttl                = 0
    default_ttl            = 86400  # 24 horas - reduce requests a S3 (Free Tier)
    max_ttl                = 31536000  # 1 año - máximo cache para reducir costos
  }
